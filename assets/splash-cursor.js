/* SplashCursor — the React Bits fluid-cursor component, ported to plain JS.
   Same simulation and the same shaders; what changed is the shell. There is no
   React here and no build step, so instead of a component with a useEffect the
   file exposes one function that builds its own canvas and hands back a
   controller:

     const fluid = SplashCursor({ ...props });
     fluid.setEnabled(true|false);   // stops the rAF loop and hides the canvas
     fluid.destroy();

   setEnabled exists because this site is one page with several states. The
   simulation should not be running -- or drawing -- while the journey video is
   playing or a case study is open, and a paused rAF costs nothing. */
window.SplashCursor = function SplashCursor(opts) {
  const o = opts || {};
  const config = {
    SIM_RESOLUTION: o.SIM_RESOLUTION ?? 128,
    DYE_RESOLUTION: o.DYE_RESOLUTION ?? 1440,
    CAPTURE_RESOLUTION: o.CAPTURE_RESOLUTION ?? 512,
    DENSITY_DISSIPATION: o.DENSITY_DISSIPATION ?? 3.5,
    VELOCITY_DISSIPATION: o.VELOCITY_DISSIPATION ?? 2,
    PRESSURE: o.PRESSURE ?? 0.1,
    PRESSURE_ITERATIONS: o.PRESSURE_ITERATIONS ?? 20,
    CURL: o.CURL ?? 3,
    SPLAT_RADIUS: o.SPLAT_RADIUS ?? 0.2,
    SPLAT_FORCE: o.SPLAT_FORCE ?? 6000,
    SHADING: o.SHADING ?? true,
    COLOR_UPDATE_SPEED: o.COLOR_UPDATE_SPEED ?? 10,
    PAUSED: false,
    BACK_COLOR: o.BACK_COLOR ?? { r: 0.5, g: 0, b: 0 },
    TRANSPARENT: o.TRANSPARENT ?? true,
    RAINBOW_MODE: o.RAINBOW_MODE ?? true,
    COLOR: o.COLOR ?? '#ff0000',
    /* An addition to the component's own props. RAINBOW_MODE gives every hue
       and COLOR gives exactly one; neither is "shades of a colour". PALETTE
       takes a list of hex values and draws from it, jittering the strength a
       little so repeated splats are not carbon copies. Set it and it wins over
       both of the above. */
  ...
  canvas.id = 'fluid';
  wrap_.appendChild(canvas);
  (o.parent || document.body).appendChild(wrap_);

  let isActive = true, enabled = false, rafId = null;
  ...
  /* No WebGL, no fluid. The page is unaffected either way, so this just leaves. */
  if (!ctx) { wrap_.remove(); return { setEnabled() {}, destroy() {} }; }
  ...
  function updateFrame() {
    if (!isActive || !enabled) { rafId = null; return; }
  ...
  function generateColor() {
    if (config.PALETTE && config.PALETTE.length) {
      const hex = config.PALETTE[(Math.random() * config.PALETTE.length) | 0];
  ...
  /* Input only counts while the fluid is on screen, so a mouse crossing the
     journey or a case study does not quietly build up a splat to be revealed
     the moment the project page appears. */
  ...
  return {
    setEnabled(on) {
      ...
      enabled = on;
      wrap_.classList.toggle('on', on);
      if (on) {
        lastUpdateTime = Date.now();   /* or the first frame arrives with a huge dt */
